...
public abstract class WebServiceClientBean implements javax.ejb.SessionBean {
	...
	public String IntegrationByXmlWithLanguageEx(String address, String xml, String language){
		...
		try {
			...
			// RETRY 3 times if execute out of time
			desc = this.getXMLDescription(result);
			if(desc.indexOf(HandleMessage.OUT_OF_TIME) > 0){
				result = stub.integrationByXmlWithLanguageEx(req).getIntegrationByXmlWithLanguageExResult();
				desc = this.getXMLDescription(result);
				if(desc.indexOf(HandleMessage.OUT_OF_TIME) > 0){
					result = stub.integrationByXmlWithLanguageEx(req).getIntegrationByXmlWithLanguageExResult();
		...
		_log.debug("Return : " + desc);
		return desc;
	}
	
	/** 
	 * Test method that always return TRUE
	 * <!-- begin-xdoclet-definition --> 
	 * @ejb.interface-method view-type="remote"
	 * <!-- end-xdoclet-definition --> 
	 * @generated
	 ...
	 * @return "0"
	 */
	public String TestTrue(String anyString) {
		return "0";
	}
	
	/** 
	 * Test method that always return FALSE
	 * <!-- begin-xdoclet-definition --> 
	 * @ejb.interface-method view-type="remote"
	 * <!-- end-xdoclet-definition --> 
	 * @generated
	 ...
	 * @return String  
	 */
	public String TestFalse(String anyString) {
		return "It is a predictable error for test" +anyString;
	}
	...
	/**
	 * Return a XML format string as response message
	 * @param info
	 * @return
	 */
	...
	/**
	 * Get the description info from XML
	 * @param xml
	 * @return
	 */
	private String getXMLDescription(String xml){
		Document doc = new XML(_log).getXMLDocUTF16(xml);

		StringBuilder sb = new StringBuilder();
		Element root = doc.getRootElement();
		Element status = root.element("Execution").element("Status");
		String code = status.attribute("Code").getText();
		if(code.equalsIgnoreCase("0")){
			return "0";
		}else if(code.equalsIgnoreCase("-1")){
			return status.attribute("description").getText();
		}
		return xml;
	}
	 ...
	 */
	private String convertUTF16toUTF8(String str) {
		try {
			str = new String(str.getBytes("UTF-16"), "UTF-8");
		} catch (UnsupportedEncodingException e) {
			LogUtil.logError(_log, "getXMLDescription() UnsupportedEncodingException.ErrMsg: " + e.getMessage());
			return responseMessage("getXMLDescription() UnsupportedEncodingException.ErrMsg: " + e.getMessage());
		}
		return str;
	}
}
